// Frank's implementation of the chess intended for transpilation into TypeScript.
//! THIS FILE IS MARKED FOR DELETION ONCE TRANSPILATION IS COMPLETE.
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int single_legality_check(Piece board[], ExtraGameInfo* game_data, Move* move_to_check, Square king_sq) {
    ...
    switch(u.move >> 12) {
        case FLAG_CASTLE_S:
            if (
                !is_in_check(board, -(game_data->side_to_move), king_sq) &&
                !is_in_check(board, -(game_data->side_to_move), (king_sq) - 1) &&
                !is_in_check(board, -(game_data->side_to_move), (king_sq) - 2)
            ) {
                return_val = 1;
            }
        case FLAG_CASTLE_L:
            ...
            break;
        default:
            if (!is_in_check(board, -(game_data->side_to_move), king_sq)) {
                return_val = 1;
            }
            break;
    }
...
void engine(Piece board[], ExtraGameInfo* game_data, short max_depth, Undo* last_move, char history[][16], Metrics* metrics) {
    int iter_depth, i;
    short engine_eval;
